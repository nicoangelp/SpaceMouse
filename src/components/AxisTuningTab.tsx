...
export const AxisTuningTab: React.FC<AxisTuningTabProps> = ({
            ...
            <div className="p-5 rounded-3xl neo-panel backdrop-blur-xl border border-transparent flex flex-wrap items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <TooltipLabel label="6-DOF Kinematics & Universal Joystick Mapping
 " tooltip="Configure response curves, deadzones, and freely remap any 6-DOF axis to volume twist, track skipping, scrolling, or custom hotkeys.
 " className="text-base font-semibold text-white tracking-tight" /></div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center p-1 rounded-xl neo-panel-inset border border-transparent gap-1">
                        <button
                            ...
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${tuningMode === 'kinematics'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${tuningMode === 'remapping'
                    ...
                    <button
                        ...
                        className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-xl bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 border border-blue-500/40 text-xs font-semibold transition active:scale-95 transition-all"
                    >
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold transition shadow-sm ${syncedRecently
                                ? 'bg-emerald-500 text-black'
                                : !isConnected
                                    ? 'neo-panel-inset text-zinc-500 border border-transparent'
                                    : 'bg-blue-600 hover:bg-cyan-400 text-black'
                            }`}
                    >
            ...
            {appliedPresetMsg && (
                <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/30 text-xs font-semibold text-blue-400 flex items-center gap-2">
                ...
                {(['x', 'y', 'z', 'rx', 'ry', 'rz'] as Array<keyof SixDofAxesConfig>).map((key) => {
                    ...
                    return (
                        <button
                            ...
                            className={`p-3.5 rounded-2xl border text-left transition-all relative overflow-hidden ${isSelected
                                    ? 'neo-panel backdrop-blur-xl border-blue-500 shadow-md shadow-sm scale-[1.02]'
                                    : 'neo-panel backdrop-blur-xl border-transparent hover:border-transparent hover:neo-panel-inset'
                                }`}
                        >
                            <div className="flex items-center justify-between">
                                <span className={`text-xs font-semibold uppercase ${isSelected ? 'text-blue-400' : 'text-white'}`}>
                                    Axis {key.toUpperCase()}
                                </span>
                                <span className="text-xs px-1.5 py-0.5 rounded neo-panel-inset border border-transparent text-zinc-400 ">
                                    {ax.outputMode === 'cad_6dof' ? `${ax.sensitivity}x` : ax.outputMode?.replace('_', ' ')}
                                </span>
                            </div>
                            <span className="text-xs text-zinc-400 font-semibold block mt-1 truncate">
            ...
            {tuningMode === 'kinematics' ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-5 space-y-4">
                        <div className="p-6 rounded-3xl neo-panel backdrop-blur-xl border border-transparent space-y-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <span className="text-xs font-semibold text-white block">Response Curve Profile</span>
                                    <span className="text-xs text-zinc-400 ">Deflection Input vs Output Velocity</span>
                                </div>
                            ...
                            <div className="relative w-full aspect-[2/1] neo-panel-inset rounded-xl border border-transparent p-3 flex items-center justify-center overflow-hidden">
                                {/* Deadzone visual zone */}
                                <div
                                    className="neo-button-danger"
                                    style={{ width: `${(currentAxis.deadzone / 100) * 85}%` }}
                                >
                                    <span className="absolute bottom-1 left-1 text-[9px] text-blue-400 font-semibold">
                            ...
                            <div className="grid grid-cols-4 gap-1.5">
                                {(['linear', 'quadratic', 'exponential', 's_curve'] as CurveType[]).map((c) => (
                                    <button
                                        ...
                                        className={`py-1.5 px-2 rounded-lg text-xs font-semibold capitalize transition ${currentAxis.curve === c
                                                ? 'bg-blue-600 text-black font-semibold shadow-sm'
                    ...
                    <div className="lg:col-span-7 space-y-4">
                        <div className="p-6 rounded-3xl neo-panel backdrop-blur-xl border border-transparent space-y-5">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-lg text-xs font-semibold transition flex items-center gap-1 ${currentAxis.inverted
                                            ? 'bg-amber-500 text-black shadow-sm'
                                            : 'neo-panel-inset border border-transparent text-zinc-400 hover:text-white'
                                        }`}
                                >
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-xs">
                                    <TooltipLabel label="Sensitivity Multiplier" tooltip="Scales the raw physical sensor deflection. Higher values increase speed. Default is 1.0x." className="font-semibold text-zinc-300" />
                                    <span className=" font-semibold text-blue-400">{currentAxis.sensitivity.toFixed(2)}x</span>
                                </div>
                                <Slider
                                    min="0.1"
                                    max="4.0"
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-xs">
                                    <span className="font-semibold text-zinc-300">Neutral Sensor Deadzone</span>
                                    <span className=" font-semibold text-blue-400">{currentAxis.deadzone}%</span>
                                </div>
                                <Slider
                                    min="0"
                                    max="35"
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-xs">
                                    <span className="font-semibold text-zinc-300">Exponential Curvature Factor (Gamma)</span>
                                    <span className=" font-semibold text-blue-400">{(currentAxis.expoPower || 2.0).toFixed(1)}</span>
                                </div>
                                <Slider
                                    min="1.0"
                                    max="3.5"
                            ...
                            <div className="pt-2 border-t border-transparent space-y-2">
                                <div className="flex items-center justify-between text-xs">
                                    <span className="font-semibold text-zinc-300">Global Low-Pass Smoothing Alpha</span>
                                    <span className=" font-semibold text-blue-400">{filters.smoothingAlpha.toFixed(2)}</span>
                                </div>
                                <Slider
                                    min="0.1"
                                    max="0.8"
                                ...
                                {AXIS_OUTPUT_MODES.map((mode) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full p-3 rounded-xl border text-left transition flex items-center justify-between ${isSelected
                                                    ? 'bg-blue-500/20 border-blue-500/40 shadow-sm'
                                                    : 'neo-panel-inset border-transparent hover:border-transparent'
                                                }`}
                                        >
                                            <div className="flex items-center gap-2.5">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isSelected ? 'bg-blue-500 text-white' : 'neo-panel backdrop-blur-xl text-zinc-400'}`}>
                                                ...
                                                <div>
                                                    ...
                                                    <span className="text-xs text-zinc-400 block mt-0.5">{mode.desc}</span>
                                                </div>
                                            </div>
                    ...
                    <div className="lg:col-span-7 space-y-4">
                        <div className="p-6 rounded-3xl neo-panel backdrop-blur-xl border border-transparent space-y-5">
                            ...
                            {currentAxis.outputMode === 'cad_6dof' && (
                                <div className="p-4 rounded-xl neo-panel-inset border border-transparent text-xs text-zinc-300 space-y-2">
                                    <span className="font-semibold text-blue-400 block">Native 3D Navigation Mode Active</span>
                                    <p>
                            ...
                            {currentAxis.outputMode === 'media_volume' && (
                                <div className="space-y-3">
                                    <div className="p-4 rounded-xl neo-panel-inset border border-transparent space-y-2">
                                        <span className="text-xs font-semibold text-white block">Volume Knob Behavior:</span>
                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                            <div className="p-2.5 rounded-lg neo-panel backdrop-blur-xl border border-blue-200">
                                                <span className="text-xs text-blue-400 font-semibold block">+ Deflection (Right / Push)</span>
                                                <span className="text-white font-semibold">Volume Up</span>
                                            </div>
                                            <div className="p-2.5 rounded-lg neo-panel backdrop-blur-xl border border-red-200">
                                                <span className="text-xs text-blue-400 font-semibold block">- Deflection (Left / Pull)</span>
                                                <span className="text-white font-semibold">Volume Down</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {currentAxis.outputMode === 'media_track' && (
                                <div className="space-y-3">
                                    <div className="p-4 rounded-xl neo-panel-inset border border-transparent space-y-2">
                                        <span className="text-xs font-semibold text-white block">Track Skip Behavior:</span>
                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                            <div className="p-2.5 rounded-lg neo-panel backdrop-blur-xl border border-blue-200">
                                                <span className="text-xs text-blue-400 font-semibold block">+ Deflection</span>
                                                <span className="text-white font-semibold">Next Track (Skip)</span>
                                            </div>
                                            <div className="p-2.5 rounded-lg neo-panel backdrop-blur-xl border border-red-200">
                                                <span className="text-xs text-blue-400 font-semibold block">- Deflection</span>
                                                <span className="text-white font-semibold">Previous Track</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {(currentAxis.outputMode === 'keystroke_repeat' || currentAxis.outputMode === 'custom_hotkey_bidirectional') && (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <div className="space-y-1">
                                            <label className="text-xs font-semibold text-blue-400">Positive (+) Action Name</label>
                                            <input
                                                type="text"
                                                value={currentAxis.positiveActionName || 'Zoom In'}
                                                onChange={(e) => onUpdateAxis(selectedAxis, { positiveActionName: e.target.value })}
                                                className="w-full px-3 py-2 rounded-lg neo-panel-inset border border-transparent text-xs text-white focus:outline-none focus:border-blue-500 font-medium"
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs font-semibold text-blue-400">Negative (-) Action Name</label>
                                            <input
                                                type="text"
                                                value={currentAxis.negativeActionName || 'Zoom Out'}
                                                onChange={(e) => onUpdateAxis(selectedAxis, { negativeActionName: e.target.value })}
                                                className="w-full px-3 py-2 rounded-lg neo-panel-inset border border-transparent text-xs text-white focus:outline-none focus:border-rose-500 font-medium"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-2 border-t border-transparent">
                                        <div className="flex items-center justify-between text-xs">
                                            <span className="font-semibold text-zinc-300">Repeat Trigger Rate</span>
                                            <span className=" font-semibold text-blue-400">{currentAxis.repeatRateMs || 80} ms</span>
                                        </div>
                                        <Slider
                                            min="20"
                                            max="400"
