...
export const SixDofGauges: React.FC<SixDofGaugesProps> = ({
  ...
  const renderGaugeBar = (
    ...
    return (
      <div className={`p-3.5 rounded-xl border transition-all ${isLocked ? 'bg-[#0a0d12]/40 border-[#1e2632]/40 opacity-50' : 'bg-[#0a0d12] border-[#1e2632] shadow-lg hover:border-cyan-500/30'}`}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className="p-1.5 rounded-lg bg-[#050608] border border-[#1e2632] text-cyan-400">{icon}</span>
            <div>
              <div className="text-xs font-bold text-slate-100 flex items-center gap-1.5 font-mono">
                <span>{label}</span>
                {config.inverted && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-950/80 text-amber-300 font-mono border border-amber-500/30">INV</span>
                )}
              </div>
          ...
          <div className="text-right">
            <span
              className={`font-mono text-xs font-extrabold ${
                isOverThreshold ? 'text-cyan-400 drop-shadow-[0_0_8px_rgba(6,182,212,0.6)]' : 'text-slate-500'
              }`}
            >
              {value >= 0 ? `+${value.toFixed(2)}` : value.toFixed(2)}
            </span>
            <div className="text-[10px] text-slate-500 font-mono">{percentage}%</div>
          </div>
        </div>
        ...
        <div className="relative w-full h-3 bg-[#050608] rounded-full overflow-hidden border border-[#1e2632] flex items-center">
          ...
          <div
            className="absolute top-0 bottom-0 bg-slate-800/80 z-0"
            style={{
        ...
        {isSimulating && (
          <div className="mt-2.5 pt-2 border-t border-[#1e2632]/80 flex items-center gap-2">
            ...
            <input
              ...
              className="w-full h-1.5 bg-[#050608] rounded-lg appearance-none cursor-pointer accent-cyan-400"
            />
            <span className="text-[10px] text-slate-500 font-mono">+1</span>
            <button
              onClick={() => onSimulateChange(axisKey, 0)}
              className="text-[10px] px-1.5 py-0.5 rounded bg-[#1e2632] text-slate-300 hover:text-white font-mono"
            >
              0
            </button>
          </div>
  ...
  return (
    <div className="w-full space-y-3.5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 bg-[#0a0d12] rounded-xl border border-[#1e2632]">
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-300 font-mono uppercase">Axis Locks:</span>
          <button
            id="toggle-pan-lock"
            onClick={() => onToggleFilter('lockPan')}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border transition-all ${
              filters.lockPan
                ? 'bg-amber-950/80 border-amber-500 text-amber-300 glow-amber-sm'
                : 'bg-[#050608] border-[#1e2632] text-slate-400 hover:text-white hover:border-slate-700'
            }`}
          >
            {filters.lockPan ? <Lock className="w-3 h-3 text-amber-400" /> : <Unlock className="w-3 h-3" />}
            <span className="font-mono text-[11px]">Lock Pan</span>
          </button>
          <button
            id="toggle-rot-lock"
          ...
          >
            {filters.lockRotation ? <Lock className="w-3 h-3 text-amber-400" /> : <Unlock className="w-3 h-3" />}
            <span className="font-mono text-[11px]">Lock Orbit</span>
          </button>
        </div>
        ...
        <button
          id="btn-zero-tare"
          onClick={onZeroTare}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-black text-xs font-bold shadow-md glow-cyan-sm transition-all active:scale-95 font-mono"
        >
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
          {renderGaugeBar('X - Pan L/R', 'x', state.x, 'bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]', 'Lateral Axis', <MoveHorizontal className="w-3 h-3 text-cyan-400" />)}
          {renderGaugeBar('Y - Pan U/D', 'y', state.y, 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]', 'Vertical Axis', <MoveVertical className="w-3 h-3 text-emerald-400" />)}
          {renderGaugeBar('Z - Zoom Dolly', 'z', state.z, 'bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.5)]', 'Depth Push/Pull', <ZoomIn className="w-3 h-3 text-purple-400" />)}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
          {renderGaugeBar('Rx - Pitch', 'rx', state.rx, 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]', 'Forward / Backward', <RotateCcw className="w-3 h-3 text-amber-400" />)}
          {renderGaugeBar('Ry - Roll', 'ry', state.ry, 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]', 'Left / Right Bank', <RotateCcw className="w-3 h-3 text-rose-400" />)}
