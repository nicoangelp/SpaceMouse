    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-white tracking-tight">
              OOFO One Key & 6-DOF Axis Remapper
            </h2>
          </div>
          <p className="text-xs text-slate-400 mt-1 max-w-2xl font-mono">
            Click any of the 9 mechanical keys or 6 joystick axes to customize tap actions, hold macros, volume knobs, scroll wheels, or repeaters.
          </p>
        </div>
        ...
        <div className="flex items-center p-1 rounded-xl bg-[#0c0e14] border border-[#232b3c] gap-1">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
              filterMode === 'all' ? 'bg-cyan-500 text-black font-bold shadow-sm' : 'text-slate-400 hover:text-white'
            }`}
          >
            All Controls
          </button>
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
              filterMode === 'keys' ? 'bg-cyan-500 text-black font-bold shadow-sm' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
              filterMode === 'axes' ? 'bg-cyan-500 text-black font-bold shadow-sm' : 'text-slate-400 hover:text-white'
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-5 rounded-2xl bg-[#141822] border border-[#232b3c] flex flex-col items-center justify-center space-y-4">
            {/* Visualizer Title */}
            <div className="w-full flex items-center justify-between text-xs text-slate-400 font-mono">
              ...
              <span className="text-[10px] text-slate-500">Click a Key or Axis Arrow</span>
            </div>

            {/* Interactive Physical Silhouette */}
            <div className="relative w-full max-w-[360px] aspect-[4/3] bg-[#0c0e14] rounded-2xl border border-[#232b3c] p-3 flex items-center justify-center shadow-inner overflow-hidden select-none">
              {/* Contoured Palm Body */}
              <div className="relative w-full h-full rounded-2xl bg-gradient-to-br from-[#181d2a] via-[#10141e] to-[#080a0f] border border-[#273248] shadow-2xl flex items-center justify-between p-3">
                ...
                <div className="w-32 h-full p-2 rounded-xl bg-[#0a0d14]/95 border border-[#1f2838] grid grid-cols-3 gap-1.5 shadow-inner">
                  {normalizedButtons.map((btn, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg text-xs font-bold font-mono transition-all flex flex-col items-center justify-center select-none shadow-sm relative group ${
                          isPressed
                            ? 'bg-amber-400 text-black ring-2 ring-amber-400 scale-95'
                            : isSelected
                            ? 'bg-cyan-500 text-black ring-2 ring-cyan-400 shadow-cyan-500/40 scale-105 z-20'
                            : 'bg-[#181d2a] text-slate-300 hover:bg-[#252e40] hover:text-white border border-[#273248]'
                        }`}
                ...
                <div className="relative w-44 h-44 flex items-center justify-center">
                  ...
                  <button
                    ...
                    className={`absolute inset-0 rounded-full border-2 transition-all flex items-center justify-between px-1 ${
                      selection.type === 'axis' && selection.axisKey === 'rz'
                        ? 'border-cyan-400 bg-cyan-500/10 shadow-[0_0_15px_rgba(6,182,212,0.4)] scale-105 z-20'
                        : 'border-cyan-500/30 hover:border-cyan-400/80 hover:bg-cyan-500/5'
                    }`}
                    title="Click to configure Axis Rz (Yaw Twist)"
                  >
                    <span className="text-[9px] font-mono text-cyan-400 font-bold bg-[#0c0e14]/90 px-1 py-0.5 rounded shadow border border-cyan-500/30 flex items-center gap-0.5">
                      <RefreshCw className="w-2.5 h-2.5" /> Rz
                    </span>
                    <span className="text-[9px] font-mono text-cyan-400 font-bold bg-[#0c0e14]/90 px-1 py-0.5 rounded shadow border border-cyan-500/30">
                      ↻
                    </span>
                  </button>
                  ...
                  <div className="absolute inset-x-0 inset-y-1 flex flex-col justify-between items-center pointer-events-none z-10">
                    <button
                      ...
                      className={`pointer-events-auto px-1.5 py-0.5 rounded-full text-[9px] font-bold font-mono transition-all ${
                        selection.type === 'axis' && selection.axisKey === 'rx'
                          ? 'bg-purple-500 text-white shadow-lg scale-110'
                          : 'bg-[#141822]/90 border border-purple-500/40 text-purple-300 hover:bg-purple-600 hover:text-white'
                      }`}
                  ...
                  <div className="absolute inset-y-0 inset-x-1 flex justify-between items-center pointer-events-none z-10">
                    <button
                      ...
                      className={`pointer-events-auto px-1 py-0.5 rounded text-[8px] font-bold font-mono transition-all ${
                        selection.type === 'axis' && selection.axisKey === 'ry'
                          ? 'bg-indigo-500 text-white shadow-lg scale-110'
                          : 'bg-[#141822]/90 border border-indigo-500/40 text-indigo-300 hover:bg-indigo-600 hover:text-white'
                      }`}
                  ...
                  <div className="w-28 h-28 rounded-full bg-gradient-to-b from-[#1c2333] to-[#0c1018] border-2 border-slate-700 shadow-xl relative flex items-center justify-center">
                    ...
                    <button
                      ...
                      className={`absolute top-1.5 px-2 py-0.5 rounded text-[9px] font-bold font-mono transition-all ${
                        selection.type === 'axis' && selection.axisKey === 'y'
                          ? 'bg-emerald-500 text-black shadow-md scale-110 z-20'
                          : 'bg-[#0a0d14]/80 text-emerald-400 hover:bg-emerald-500 hover:text-black border border-emerald-500/30'
                      }`}
                    ...
                    <button
                      onClick={() => setSelection({ type: 'axis', axisKey: 'y' })}
                      className={`absolute bottom-1.5 px-2 py-0.5 rounded text-[9px] font-bold font-mono transition-all ${
                        selection.type === 'axis' && selection.axisKey === 'y'
                          ? 'bg-emerald-500 text-black shadow-md scale-110 z-20'
                          : 'bg-[#0a0d14]/80 text-emerald-400 hover:bg-emerald-500 hover:text-black border border-emerald-500/30'
                      }`}
                    ...
                    <button
                      ...
                      className={`absolute left-1.5 px-1.5 py-0.5 rounded text-[9px] font-bold font-mono transition-all ${
                        selection.type === 'axis' && selection.axisKey === 'x'
                          ? 'bg-sky-500 text-black shadow-md scale-110 z-20'
                          : 'bg-[#0a0d14]/80 text-sky-400 hover:bg-sky-500 hover:text-black border border-sky-500/30'
                      }`}
                    ...
                    <button
                      onClick={() => setSelection({ type: 'axis', axisKey: 'x' })}
                      className={`absolute right-1.5 px-1.5 py-0.5 rounded text-[9px] font-bold font-mono transition-all ${
                        selection.type === 'axis' && selection.axisKey === 'x'
                          ? 'bg-sky-500 text-black shadow-md scale-110 z-20'
                          : 'bg-[#0a0d14]/80 text-sky-400 hover:bg-sky-500 hover:text-black border border-sky-500/30'
                      }`}
                    ...
                    <button
                      ...
                      className={`w-12 h-12 rounded-full border transition-all flex flex-col items-center justify-center shadow-inner ${
                        selection.type === 'axis' && selection.axisKey === 'z'
                          ? 'bg-amber-500 text-black border-amber-300 shadow-amber-500/50 scale-110 z-20 font-black'
                          : 'bg-[#141822] text-amber-400 hover:bg-amber-500/20 border-amber-500/40 font-bold'
                      }`}
                      title="Click to configure Axis Z (Zoom Push / Pull)"
                    >
                      <span className="text-[10px] leading-none">↕ Z</span>
            ...
            <div className="w-full space-y-2 pt-1">
              ...
              <div className="flex flex-wrap items-center gap-1 justify-center">
                <span className="text-[10px] text-slate-500 font-mono mr-1">KEYS:</span>
              ...
              {/* 6 Axis Pills */}
              <div className="flex flex-wrap items-center gap-1 justify-center">
                <span className="text-[10px] text-slate-500 font-mono mr-1">AXES:</span>
            ...
            <div className="w-full p-3 rounded-xl bg-[#0c0e14] border border-[#232b3c] flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                <span className="w-7 h-7 rounded-lg bg-cyan-500 text-black font-black flex items-center justify-center text-xs font-mono">
              ...
              {selection.type === 'key' && (
                <div className="flex items-center gap-1 bg-[#141822] px-2 py-1 rounded-lg border border-[#232b3c]">
                  <span className="text-[9px] text-slate-400 font-mono">PIN:</span>
                  <select
                    value={activeBtn.pinNumber}
                    onChange={(e) => handlePinChange(parseInt(e.target.value, 10))}
                    className="bg-transparent text-xs font-bold text-cyan-400 focus:outline-none cursor-pointer font-mono"
                  >
                    {AVAILABLE_GPIO_PINS.map((pin) => (
                      <option key={pin} value={pin} className="bg-[#141822] text-white">
                        GPIO {pin}
                      </option>
                    ))}
                  </select>
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {selection.type === 'key' && (
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-[#141822] border border-[#232b3c] flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Fingerprint className="w-4 h-4 text-cyan-400" />
                  <span className="text-xs font-bold text-white">Configure Key {selectedKeyIndex + 1} Action:</span>
                </div>
              ...
              <div className="p-3.5 rounded-xl bg-[#0c0e14] border border-[#232b3c] grid grid-cols-2 gap-3 text-xs">
                <div className={`p-2.5 rounded-lg border transition ${activeActionTarget === 'tap' ? 'bg-cyan-950/40 border-cyan-500/60' : 'bg-[#141822] border-[#232b3c]'}`}>
                ...
                <div className={`p-2.5 rounded-lg border transition ${activeActionTarget === 'hold' ? 'bg-purple-950/40 border-purple-500/60' : 'bg-[#141822] border-[#232b3c]'}`}>
              ...
              <div className="p-1 rounded-xl bg-[#141822] border border-[#232b3c] flex flex-wrap gap-1">
                {[
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {WINDOWS_SHORTCUTS.map((item, idx) => (
                    <button
                      ...
                      className="p-3 rounded-xl bg-[#141822] border border-[#232b3c] hover:border-cyan-500/50 hover:bg-[#1a202c] text-left transition flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-[#0c0e14] border border-[#232b3c] flex items-center justify-center text-cyan-400">
                          <item.icon className="w-4 h-4" />
                        </div>
                        <div>
                          <span className="text-xs font-bold text-white block group-hover:text-cyan-400 transition">{item.name}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{item.keys.join(' + ')}</span>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {MEDIA_SHORTCUTS.map((item, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleAssignKeyShortcut(item.name, item.keys)}
                      className="p-3 rounded-xl bg-[#141822] border border-[#232b3c] hover:border-purple-500/50 hover:bg-[#1a202c] text-left transition flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-[#0c0e14] border border-[#232b3c] flex items-center justify-center text-purple-400">
                          <item.icon className="w-4 h-4" />
                        </div>
                        <div>
                          <span className="text-xs font-bold text-white block group-hover:text-purple-400 transition">{item.name}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{item.keys.join(' + ')}</span>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {WEB_SHORTCUTS.map((item, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleAssignKeyShortcut(item.name, item.keys)}
                      className="p-3 rounded-xl bg-[#141822] border border-[#232b3c] hover:border-emerald-500/50 hover:bg-[#1a202c] text-left transition flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-[#0c0e14] border border-[#232b3c] flex items-center justify-center text-emerald-400">
                          <item.icon className="w-4 h-4" />
                        </div>
                        <div>
                          <span className="text-xs font-bold text-white block group-hover:text-emerald-400 transition">{item.name}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{item.keys.join(' + ')}</span>
                        </div>
                      </div>
                  ...
                  {HARDWARE_ACTIONS.map((item, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className="p-3 rounded-xl bg-[#141822] border border-[#232b3c] hover:border-amber-500/50 hover:bg-[#1a202c] text-left transition flex items-center justify-between group"
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-lg bg-[#0c0e14] border border-[#232b3c] flex items-center justify-center text-amber-400">
                            <Icon className="w-4 h-4" />
                          </div>
                          <div>
                            <span className="text-xs font-bold text-white block group-hover:text-amber-400 transition">{item.name}</span>
                            <span className="text-[10px] text-slate-400 block">{item.desc}</span>
                          </div>
                        </div>
                        <span className="text-[10px] px-2 py-1 rounded bg-[#0c0e14] text-slate-400 font-mono">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-[320px] overflow-y-auto pr-1">
                    {savedCombos.map((combo) => (
                      <div
                        key={combo.id}
                        className="p-3 rounded-xl bg-[#141822] border border-[#232b3c] flex items-center justify-between"
                      >
                        <div
                          onClick={() => handleAssignKeyShortcut(combo.name, combo.keys)}
                          className="cursor-pointer flex-1"
                        >
                          <span className="text-xs font-bold text-white hover:text-cyan-400 transition block">{combo.name}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{combo.keys.join(' + ')}</span>
                        </div>
                        <button
              ...
              {activeKeyCategory === 'custom_recorder' && (
                <div className="p-5 rounded-2xl bg-[#141822] border border-[#232b3c] space-y-4">
                  ...
                  <div className="p-4 rounded-xl bg-[#0c0e14] border border-[#232b3c] flex flex-col items-center justify-center space-y-3">
                    <div className="flex items-center gap-2 min-h-[36px]">
                      {recordedKeys.length > 0 ? (
                        recordedKeys.map((k, idx) => (
                          <span key={idx} className="px-2.5 py-1 rounded-lg bg-cyan-500 text-black font-black text-xs font-mono shadow">
                            {k}
                          </span>
                        ))
                      ) : (
                    ...
                    <div className="flex items-center gap-2">
                      {!isRecording ? (
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-bold text-xs shadow-md glow-cyan-sm transition"
                        >
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold text-xs shadow-md transition animate-pulse"
                        >
                          Stop Recording
                        </button>
                      )}
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded-xl bg-[#0c0e14] border border-[#232b3c] text-xs text-white focus:outline-none focus:border-cyan-500"
                    />
                    <button
                      onClick={handleSaveRecordedCombo}
                      disabled={recordedKeys.length === 0}
                      className="px-4 py-2 rounded-xl bg-emerald-500 disabled:opacity-40 hover:bg-emerald-400 text-black font-bold text-xs shadow transition flex items-center gap-1.5"
                    >
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-[#141822] border border-[#232b3c] space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {/* Mode Badge */}
                  <span className="text-[10px] px-2.5 py-1 rounded-full bg-purple-950/80 border border-purple-500/40 text-purple-300 font-bold font-mono">
                    Mode: {currentAxisConfig.outputMode || 'cad_6dof'}
                  </span>
                </div>

                {/* Quick Axis Preset Pills */}
                <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-[#232b3c]">
                  <span className="text-[10px] text-slate-500 font-mono">QUICK PRESETS:</span>
                  <button
                    onClick={() => handleApplyAxisPreset('volume')}
                    className="px-2.5 py-1 rounded-lg bg-[#0c0e14] hover:bg-purple-950/60 text-purple-300 hover:text-white text-[11px] font-medium border border-purple-500/30 flex items-center gap-1 transition"
                  >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-[#0c0e14] hover:bg-cyan-950/60 text-cyan-300 hover:text-white text-[11px] font-medium border border-cyan-500/30 flex items-center gap-1 transition"
                  >
                    <ArrowUpDown className="w-3 h-3" /> Scroll Wheel
                  </button>
                  <button
                    onClick={() => handleApplyAxisPreset('zoom_keys')}
                    className="px-2.5 py-1 rounded-lg bg-[#0c0e14] hover:bg-amber-950/60 text-amber-300 hover:text-white text-[11px] font-medium border border-amber-500/30 flex items-center gap-1 transition"
                  >
                    <Keyboard className="w-3 h-3" /> Zoom Ctrl+/-
                  </button>
                  <button
                    onClick={() => handleApplyAxisPreset('cad')}
                    className="px-2.5 py-1 rounded-lg bg-[#0c0e14] hover:bg-cyan-950/60 text-slate-300 hover:text-white text-[11px] font-medium border border-slate-700 flex items-center gap-1 transition"
                  >
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-xl border text-left transition flex items-start gap-3 ${
                          isSelected
                            ? 'bg-purple-950/50 border-purple-400 shadow-md ring-1 ring-purple-400/50'
                            : 'bg-[#141822] border-[#232b3c] hover:border-slate-600 hover:bg-[#1a202c]'
                        }`}
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                          isSelected ? 'bg-purple-500 text-white' : 'bg-[#0c0e14] text-slate-400 border border-[#232b3c]'
                        }`}>
                          <Icon className="w-4 h-4" />
                        </div>
              ...
              {['keystroke_repeat', 'custom_hotkey_bidirectional', 'media_volume', 'media_track'].includes(currentAxisConfig.outputMode || '') && (
                <div className="p-4 rounded-2xl bg-[#141822] border border-[#232b3c] space-y-4">
                  <span className="text-xs font-bold text-white flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {/* Positive Deflection */}
                    <div className="p-3 rounded-xl bg-[#0c0e14] border border-[#232b3c] space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-2.5 py-1.5 rounded-lg bg-[#141822] border border-[#232b3c] text-xs text-white focus:outline-none focus:border-emerald-500"
                      />
                      <div className="flex items-center justify-between pt-1">
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded bg-emerald-500/20 hover:bg-emerald-500 text-emerald-300 hover:text-black text-[10px] font-bold transition font-mono"
                        >
                          Record Keys
                        </button>
                      </div>
                    ...
                    <div className="p-3 rounded-xl bg-[#0c0e14] border border-[#232b3c] space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-2.5 py-1.5 rounded-lg bg-[#141822] border border-[#232b3c] text-xs text-white focus:outline-none focus:border-rose-500"
                      />
                      <div className="flex items-center justify-between pt-1">
                        <span className="text-[10px] text-slate-400 font-mono">
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded bg-rose-500/20 hover:bg-rose-500 text-rose-300 hover:text-black text-[10px] font-bold transition font-mono"
                        >
                          Record Keys
                        </button>
                      </div>
                  ...
                  <div className="space-y-2 pt-2 border-t border-[#232b3c]">
                    ...
                    <div className="flex items-center gap-2 justify-end text-[10px]">
                      <button
                        ...
                        className="px-2 py-0.5 rounded bg-[#0c0e14] text-slate-400 hover:text-cyan-400 font-mono"
                      >
                        ⚡ 35ms (Rapid)
                      </button>
                      <button
              ...
              <div className="p-4 rounded-2xl bg-[#141822] border border-[#232b3c] space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  {/* Invert Direction */}
                  <div className="p-3 rounded-xl bg-[#0c0e14] border border-[#232b3c] flex items-center justify-between">
                    <div>
                      <span className="font-bold text-white block">Invert Axis Direction</span>
                      <span className="text-[10px] text-slate-400">Swap positive and negative deflection</span>
                    </div>
                    <button
                      onClick={() => onUpdateAxis && onUpdateAxis(selectedAxisKey, { inverted: !currentAxisConfig.inverted })}
                      className={`w-12 h-6 rounded-full transition-colors relative p-0.5 ${
                        currentAxisConfig.inverted ? 'bg-purple-500' : 'bg-slate-700'
                      }`}
                    >
                  ...
                  {/* Deadzone */}
                  <div className="p-3 rounded-xl bg-[#0c0e14] border border-[#232b3c] space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-white">Deadzone</span>
                      <span className="font-mono text-purple-400 font-bold">{currentAxisConfig.deadzone}%</span>
                    </div>
                    <input
                      type="range"
                      min="1"
